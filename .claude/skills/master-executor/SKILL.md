...
- **Own skill**: Always load `master-executor` skill first.
- **Helper skills**: Load at most 2 helpers per task, only when triggered.
  - Triggered helpers: `systematic-debugging`, `verification-before-completion`
...
## BƯỚC 1 — ĐỌC AGENTS.md VÀ PROJECT CONTEXT

```
Read: AGENTS.md (nếu tồn tại)
Read: .Codex/settings.json (nếu tồn tại)
```
...
Nếu AGENTS.md không tồn tại → tiếp tục, nhưng thận trọng hơn với conventions.
...
- Sửa `AGENTS.md` hoặc `.Codex/` settings trừ khi user yêu cầu trực tiếp
