...
MM_CLAUDE="$HOME/.claude/bin/mm-claude.sh"
MM_VLM="$HOME/.claude/bin/mm-vlm.sh"
MM_SEARCH="$HOME/.claude/bin/mm-search.sh"
MM_IMAGE="$HOME/.claude/bin/mm-image.sh"
MM_AUDIO="$HOME/.claude/bin/mm-audio.sh"
...
# Resolve capability upgrades
if [ "$UPGRADE_SEARCH" -eq 1 ]; then
  REQUESTED_CAPABILITY="search"
  case "$ROLE" in
    security-auditor|design-finder) RESOLVED_EXECUTOR="$MM_SEARCH" ;;
