...
Dựa trên **MiniMax M2.7 Playbook §5.2** (hard rules) và **CLAUDE.md global orchestration**:
...
```bash
bash ~/.claude/bin/mm-delegate-phase.sh <plan-file>
```

Script này (xem §8) wrap `mm-claude.sh` với logging + status capture.
...
File template: `D:\NELSON\2. Areas\.claude\skills\workflow\templates\mm-phase-plan.md`
...
- Log mỗi delegation vào `~/.claude/mm-wf-runs.log`
...
Wrapper quanh `mm-claude.sh` với structured logging:

**Location:** `C:\Users\Nelson\.claude\bin\mm-delegate-phase.sh`

Ví dụ call:
```bash
bash ~/.claude/bin/mm-delegate-phase.sh /tmp/mm-wf-master-executor-260425.md
```

Script tự động:
- Log start timestamp vào `~/.claude/mm-wf-runs.log`
- Call `mm-claude.sh --file <plan>`
...
# 2. Bash("bash ~/.claude/bin/mm-delegate-phase.sh /tmp/mm-wf-master-executor-260425.md")
# 3. Parse output
# 4. Bash("git diff --stat")
```

Các skills được M2.7 invoke cũng có full tool access (vì M2.7 chạy qua Claude Code CLI với cùng toolset).
